...
impl App<'_> {
    pub(super) fn render_form_body_tab(
        &mut self,
        frame: &mut Frame,
        area: Rect,
        form: &Vec<KeyValue>,
        ...
        frame.render_stateful_widget(
            left_list,
            table_layout[0],
            &mut self.body_form_table.left_state.clone(),
        );
        frame.render_stateful_widget(
            right_list,
            table_layout[1],
            &mut self.body_form_table.right_state.clone(),
        );
        ...
        if self.state == EditingRequestBodyTable {
            ...
            let height_adjustment = (form_selection.0 - self.body_form_table.left_state.offset())
                as u16
                % form_layout[1].height;
            ...
            let text_input = Paragraph::new(format!(
                "{:fill$}",
                form_data_text,
                fill = (cell_with - horizontal_margin) as usize
            ));
            ...
            frame.set_cursor(
                selection_position_x
                    + self.body_form_table.selection_text_input.cursor_position as u16,
