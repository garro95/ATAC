...
impl App<'_> {
    pub fn render_request_params(&mut self, frame: &mut Frame, rect: Rect, request: &Request) {
        ...
        match self.request_param_tab {
            RequestParamsTabs::QueryParams => match self.query_params_table.selection {
                None => {
            ...
            RequestParamsTabs::Headers => match self.headers_table.selection {
                None => {
            ...
            RequestParamsTabs::Body => match &request.body {
                ...
                Multipart(form) | Form(form) => match self.body_form_table.selection {
                    None => {
                        ...
                        let multipart_form_paragraph =
                            Paragraph::new(multipart_form_lines).centered();

                        frame.render_widget(multipart_form_paragraph, request_params_layout[1]);
                    }
                    Some(multipart_form_selection) => {
                        self.render_form_body_tab(
                            frame,
                            request_params_layout[1],
                            form,
                            multipart_form_selection,
                        );
                    }
                },
                File(_) => {
                ...
                Raw(_) | Json(_) | Xml(_) | Html(_) | Javascript(_) => {
                    self.body_text_area
                        .set_line_number_style(Style::new().fg(Color::DarkGray));
