...
impl App<'_> {
    pub(super) fn render_query_params_tab(
        &mut self,
        frame: &mut Frame,
        area: Rect,
        request: &Request,
        param_selection: (usize, usize),
    ) {
        let params_layout =
        ...
        let left_list = List::new(params).highlight_style(left_list_style);
        ...
        frame.render_stateful_widget(
            left_list,
            table_layout[0],
            &mut self.query_params_table.left_state.clone(),
        );
        frame.render_stateful_widget(
            right_list,
            table_layout[1],
            &mut self.query_params_table.right_state.clone(),
        );
        ...
        if self.state == EditingRequestParam {
            ...
            let height_adjustment =
                (param_selection.0 - self.query_params_table.left_state.offset()) as u16
                    % params_layout[1].height;
