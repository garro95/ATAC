...
use crate::app::app_states::AppState::{
    EditingRequestAuthPassword, EditingRequestAuthUsername, SelectedRequest,
};
...
impl App<'_> {
    pub(super) fn render_basic_auth_tab(&mut self, frame: &mut Frame, area: Rect) {
        let basic_auth_layout =
            Layout::new(Vertical, [Constraint::Length(3), Constraint::Length(3)])
                .vertical_margin(1)
                .horizontal_margin(4)
                .split(area);

        let mut username_block = Block::new().title("Username").borders(Borders::ALL);

        let mut password_block = Block::new().title("Password").borders(Borders::ALL);
        ...
        let (username_padded_text, username_input_cursor_position) = self
            .auth_basic_username_text_input
            .get_padded_text_and_cursor(username_adjusted_input_length);

        let password_adjusted_input_length = basic_auth_layout[1].width as usize - 2;
        let (password_padded_text, password_input_cursor_position) = self
            .auth_basic_password_text_input
            .get_padded_text_and_cursor(password_adjusted_input_length);
