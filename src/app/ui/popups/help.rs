...
use crate::app::app_states::event_available_keys_to_spans;
...
impl App<'_> {
    pub fn render_help_popup(&mut self, frame: &mut Frame) {
        ...
        let title_paragraph =
            Paragraph::new(self.help_popup.selection.to_string().bold().underlined()).centered();
        ...
        for i in 0..3 * NB_LINES {
            ...
            match i {
                ...
                NB_LINES..=MIDDLE_MAX => {
                    frame
                        .render_widget(keys_paragraph, middle_layout[(i - NB_LINES) * LINE_LENGTH]);
                }
                RIGHT_MIN..=RIGHT_MAX => {
                    frame.render_widget(
                        keys_paragraph,
                        right_layout[(i - 2 * NB_LINES) * LINE_LENGTH],
                    );
                }
                _ => {}
            }
