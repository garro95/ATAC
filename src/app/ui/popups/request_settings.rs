...
impl App<'_> {
    pub fn render_request_settings_popup(&mut self, frame: &mut Frame) {
        ...
        let settings_names_layout = Layout::new(Vertical, constraints.clone())
            .horizontal_margin(1)
            .split(request_settings_layout[0]);

        let settings_values_layout =
            Layout::new(Vertical, constraints).split(request_settings_layout[1]);

        // Render settings
        for (index, (setting_name, setting_value)) in
            self.request_settings_popup.settings.iter().enumerate()
        {
