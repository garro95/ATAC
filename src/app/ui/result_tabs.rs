...
impl App<'_> {
    pub(super) fn render_request_result(
        ...
        // If the selected request is not pending
            ...
            match self.request_result_tab {
                ...
                RequestResultTabs::Cookies => {
                    ...
                    let cookies_paragraph = Paragraph::new(result_cookies).scroll((
                        self.result_vertical_scrollbar.scroll,
                        self.result_horizontal_scrollbar.scroll,
                    ));
                ...
                RequestResultTabs::Headers => {
                    let result_headers: Vec<Line> = request
                        ...
                        .map(|(header, value)| {
                            Line::from(vec![
                                Span::raw(header).bold().dark_gray(),
                                Span::raw(": "),
                                Span::raw(value),
                            ])
                        })
                ...
                RequestResultTabs::Console => {
                    let highlighted_console_output = self
                        .syntax_highlighting
                        .highlighted_console_output
                        .read()
                        .clone();

                    let console_paragraph = Paragraph::new(highlighted_console_output).scroll((
                        self.result_vertical_scrollbar.scroll,
                        self.result_horizontal_scrollbar.scroll,
                    ));
        ...
        let result_horizontal_scrollbar =
            Scrollbar::new(ScrollbarOrientation::HorizontalBottom).thumb_symbol("■"); // Better than the default full block
